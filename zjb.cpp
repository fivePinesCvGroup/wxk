...
while (true)
{
    ...
    waitKey(1);
    img1. copyTo(copy);
    cvtColor(img1 , mask , COLOR_BGR2HSV);
    ...
    for(const auto & contour: lightContours)
    {
        ...
        if(lightRec.size.width / lightRec.size.height > light_width_height_ratio || lightContourArea/ lightRec.size.area() < light_contour_rect_ratio)
            continue;
